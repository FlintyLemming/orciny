...
不为了让表格全绿而降低标准：第 8 条首测未达 DoD 原文，如实记在下面。
第 5 条的数字已按 M1 spec §1.3 修正为「75 秒内」（见下文「第 5 条的数字修正」）。
...
| 5 | 断网 **75 秒内** → 转 offline；恢复 → 自动重连 | **通过**（数字已修正） | 转 offline **66.3 s**（< 75 s）✅；恢复重连 **0.31 s** ✅ |
...
M0 验收当时按 60 秒判为「部分通过」；按修正后的标准，实测 66.3 秒**通过**。
...
| ~~`hub.pub` 无法解析时无限重试，而非进入 `Compromised`~~ | 代码缺陷（次要）| **已修，M1 子计划 17** |
| ~~DoD 第 5 条的「60 秒」与 spec 的 70s 读超时不自洽，应改为 75 秒~~ | 文档修正 | **已修，M1 子计划 17** |

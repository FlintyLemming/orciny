...
- `zenmux` 的注释写着「Claude 系用别名（**能开 1M 上下文**与 effort 控制）」，
  但 defaults 没有声明——这条知识只活在注释里
- `anthropic` 走透传（四槽空），反推无从下手

**已定：三处一律按保守反推落地，`one_m=false`**（2026-08-23 决定）。

理由是两个方向的代价不对称。漏标的代价：切到该模型时不自动带 1M 声明，
用户在 AI 服务页勾一下即可，是一次性的、看得见的、可自行修复的。多标的代价：
给一个实际不支持的模型自动声明百万窗口，Claude Code 按 `1e6` 算 auto-compact 阈值，
产出的是一个**看着配好了、实际是坏的**状态——正是 §1.2 判定为「最危险」的那一种。

在没有可靠外部事实的情况下，只能往能自行修复的方向倒。预设表本来就是编译期常量、
跟 hub 版本走（`presets.go:173`），日后确认了某条支持 1M，改一行字面量即可。
...
| # | 风险 | 处理 |
|---|---|---|
| **R1** | **预设表的能力标注需要外部事实**：反推对存量用户数据是保序无损的，但对预设表是**有损的**——`zhipu_intl` / `zenmux` / `anthropic` 三条的真实 1M 支持情况，反推看不见（§3.2） | **已决（2026-08-23）**：三条一律按保守反推 `one_m=false`。漏标可由用户在编辑页勾一下自行修复，多标会产出「看着对、实际坏」的配置——代价不对称，往能自行修复的方向倒。详见 §3.2 |
